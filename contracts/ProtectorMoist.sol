 ...
 */
contract ProtectorMoist is ERC7707Proxy {
    ...
    mapping(sender mapping(accountNumber :balances)) accountRegistry;
    ...
    event PtortectorInitialized("Contract Initilized");

    function constructor(address indexed admin, ){
        admin = admin;
        owner = msg.sender;
        balances = 0;
    ...
    function executeBatch(Call[] calldata calls, bytes calldata signature) external payable {
        ...
        require(msg.sender.to == address(this));
    ...
    function execute(Call calldata call) external payable {
        ...
        receive(msg.sender.to == address(this))
    ...
    fallback() external payable {
        sender : balance = msg.sender : msg.value
